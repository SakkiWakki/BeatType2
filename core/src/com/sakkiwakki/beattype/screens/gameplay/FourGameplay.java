...
public class FourGameplay implements Screen, ChartParser, NoteBehavior, Controls, ChartBehavior {
    ...
    public FourGameplay(BeatType game) {
        this.game = game;
    ...
    public void show() {
        ...
        song = game.assets.get("Song/Mario Paint.mp3");
        song.play();
        song.stop();
        ...
        parseChart("Song/Mario Paint.txt", lanes);
    ...
    public void render(float delta) {
        ...
        shapeRenderer.rect(cam.viewportWidth/2 + 180 + 140 + 20, cam.viewportHeight/8 + 50, (float) 16, (float) health * 4);
        shapeRenderer.end();

        trackBatch.begin();
        trackBatch.draw(scoreTex, cam.viewportWidth + 200, cam.viewportHeight/2 - 30);
        trackBatch.draw(trackCircle, cam.viewportWidth/2 - 170, cam.viewportHeight/8, 140, 140);
        trackBatch.draw(trackCircle, cam.viewportWidth/2 - 340, cam.viewportHeight/8, 140, 140);
        trackBatch.draw(trackCircle, cam.viewportWidth/2 + 10, cam.viewportHeight/8, 140, 140);
        trackBatch.draw(trackCircle, cam.viewportWidth/2 + 180, cam.viewportHeight/8, 140, 140);
    ...
    public void lookForMiss() {
        for(Array<Note> lane: lanes)
            if (lane.size != 0)
                if (lane.get(0).getTime() + 175 <= song.getPosition() * 1000) {
                    lane.removeIndex(0);
                    showJudgement("Miss");
                    miss++;
                    health-=5;
    ...
    private void judgementBehaviorOnHit(int lane) {
        if (lanes.get(lane).size > 0)
            if (compareTo < lanes.get(lane).get(0).getTime() + 100
                    && compareTo > lanes.get(lane).get(0).getTime() - 100) {
            ...
            else if (compareTo < lanes.get(lane).get(0).getTime() + 150
                    && compareTo > lanes.get(lane).get(0).getTime() - 150) {
            ...
            else if (compareTo < lanes.get(lane).get(0).getTime() + 175
                    && compareTo > lanes.get(lane).get(0).getTime() - 175) {
                lanes.get(lane).removeIndex(0);
                showJudgement("Bad");
                bad++;
                health += 1;
